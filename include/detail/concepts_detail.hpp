...
struct is_valid_helper<F, X, std::void_t<std::result_of_t<F(X)>>>
    : std::true_type {};

template <typename> struct member_ptr_helper {};

template <typename U, typename T> struct member_ptr_helper<U T::*> {
    using type = U;
};
